export default function About({ strengths }) {
  return (
    <section id="about" className="grid gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
      <div
        data-reveal="left"
        className="rounded-[1.75rem] border border-slate-200/80 bg-slate-950 p-8 text-white shadow-[0_18px_50px_rgba(15,23,42,0.12)] dark:border-white/10 dark:bg-cyan-400 dark:text-slate-950"
      >
      ...
      <div data-reveal="right" className="grid gap-4" style={{ "--reveal-delay": "100ms" }}>
        {strengths.map((item, index) => (
          <article
            key={item.title}
            style={{ "--reveal-delay": `${180 + index * 90}ms` }}
            className="interactive-card rounded-[1.6rem] border border-slate-200/80 bg-white p-6 shadow-[0_14px_40px_rgba(15,23,42,0.06)] dark:border-white/10 dark:bg-slate-900"
          >
